...
const Cart = ({ match, history }) => {
    ...
    React.useEffect(()=>{
        ...
        axios.get(`https://fcs.myonlineedu.in:7000/cart/${id}`, {headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{
    ...
    const releaseCart = async(d) => {
        ...
        const id = localStorage.getItem('userId');
        axios.post(`https://fcs.myonlineedu.in:7000/cart/remove/${id}`,d,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{
            // setData(res.data[0].products) 
            // console.log(res.data[0].products, "{{}}")
        });
        setRedirect(true);
