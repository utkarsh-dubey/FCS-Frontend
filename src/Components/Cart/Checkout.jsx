...
const Checkout = ({ match, history, setSessionId }) => {
    ...
    React.useEffect(()=>{
        ...
        // axios.get(`/cart/checkout/${id}`)
        // axios.get(`/cart/checkout/${id}`)
        // axios.get(`/cart/checkout/${id}`)
        axios.get(`/cart/checkout/${id}`,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{
            setData(res.data[0].products) 
            // console.log(res)
            // console.log(res.data[0].products, "{{}}")
        });
        axios.get(`/address/${id}`,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{
            console.log(res)
            setAddress(res.data)
        });
    }, [])
