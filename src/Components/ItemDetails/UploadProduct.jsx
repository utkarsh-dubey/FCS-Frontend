...
const UploadProduct = ({setPdfid}) => {
  ...
  const [page, setPage] = useState(0);
    React.useEffect(()=>{
        ...
        axios.get(`https://54.85.12.94:7000/pdf/${userId}`,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{console.log(res.data);setMainData(res.data)})
    }, [])
    ...
    const handleSubmit = () => {
        ...
        axios.post(`https://54.85.12.94:7000/product/pdfupload/${userId}`, formData,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{console.log(res.data);window.alert("PDF uploaded")})
    }
