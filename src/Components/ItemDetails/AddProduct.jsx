...
const AddProduct = ({pdfid}) => {
    ...
    const handleClick = () =>{
        ...
        axios.post('https://54.85.12.94:7000/product/imageupload', formData,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{setImages(res.data.links);window.alert("Images uploaded")});
    }
    ...
    images: images
        ...
        axios.post(`https://54.85.12.94:7000/product/add/${userId}?pdfId=${pdfid}`, payload,{headers: { Authorization: `Bearer ${localStorage.getItem("auth_token")}`}}).then(res=>{window.alert("Product created successfully")})
    }
